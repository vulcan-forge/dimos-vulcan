...
def _global_map_colors(cloud):  # type: ignore[no-untyped-def]
    ...
    colors[~obstacle_mask] = np.array([90, 150, 255], dtype=np.uint8)
    colors[obstacle_mask] = np.array([0, 255, 70], dtype=np.uint8)
    radii = np.where(obstacle_mask, 0.028, 0.03).astype(np.float32)
...
def _global_costmap_mesh(grid):  # type: ignore[no-untyped-def]
    remembered_obstacles = grid.filter_above(0).max()
    return remembered_obstacles.to_rerun(
        colormap="Greens",
        background="#000000",
        opacity=0.98,
        cost_range=(100, 100),
